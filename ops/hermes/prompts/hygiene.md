...
2. List open Linear issues labelled `hermes`. For each: does the cited `path:line` still match at the current SHA? Has an open/merged PR addressed it? Has a human commented?
...
5. Reply with a summary (≤15 lines). Only if the `mcp_hindsight_*` calls themselves failed may the FIRST line be "MEMORY TIER UNAVAILABLE".
